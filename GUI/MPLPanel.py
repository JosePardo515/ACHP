...
class MPLPanel(wx.Panel):
    ...
    def TSOverlay(self,Cycle,**kwargs):
        ...
        for i in np.arange(len(Tsat)):
            ssatL[i] = PropsSI('S','T',Tsat[i],'Q',0,Ref)
            ssatV[i] = PropsSI('S','T',Tsat[i],'Q',1,Ref)
    
        self.axes.plot(ssatL/1000,Tsat,'k')
        self.axes.plot(ssatV/1000,Tsat,'k')
        ...
        if (Cycle.CycleType=='Secondary' and Cycle.Mode=='AC') or Cycle.CycleType=='DX':
            sL=PropsSI('S','T',Cycle.Tdew_cond,'Q',0,Ref)
            sV=PropsSI('S','T',Cycle.Tdew_cond,'Q',1,Ref)
            s_cond=np.r_[Cycle.Condenser.sin_r/1000.,sV/1000.,sL/1000.,Cycle.Condenser.sout_r/1000.]
            ...
            sV=PropsSI('S','T',Cycle.Tdew_evap,'Q',1,Ref)
        ...
        if Cycle.CycleType=="Secondary":
            if Cycle.IHXType=='Coaxial':
                ...
                if Cycle.Mode=='AC':
                    ...
                    IHX=Cycle.PHEIHX
                    sL=PropsSI('S','T',Cycle.Tbubble_cond,'Q',0,Ref)
                    sV=PropsSI('S','T',Cycle.Tdew_cond,'Q',1,Ref)
            ...
            sL=PropsSI('S','T',Cycle.Tdew_evap,'Q',0,Ref)
            sV=PropsSI('S','T',Cycle.Tdew_evap,'Q',1,Ref)
    ...
    def PHOverlay(self,Cycle,**kwargs):
        ...
        for i in np.arange(len(Tsat)):
            psatL[i] = PropsSI('P','T',Tsat[i],'Q',0,Ref)
            psatV[i] = PropsSI('P','T',Tsat[i],'Q',1,Ref)
